    ...
    <section id="numbers" className="band bg-canvas">
      <div className="edge">
        ...
        <p data-reveal style={revealDelay(2)} className="lead mt-6 max-w-[36rem]">
          정원과 장비와 사람 수는 계획서에 적힌 포부가 아니라 이미 확정된 숫자예요.
          네 개만 보면 규모가 잡혀요.
        </p>
        ...
        <div
          data-reveal
          style={revealDelay(3)}
          className="mt-12 rounded-[--radius-xl] bg-brand-strong px-6 py-10 md:mt-16 md:px-12 md:py-14"
        >
        ...
        <ul
          data-reveal
          style={revealDelay(4)}
          className="card mt-3 grid divide-y divide-line md:grid-cols-3 md:divide-x md:divide-y-0"
        >
          {REST.map((n) => (
        ...
        >
          <figure className="flex items-center justify-center rounded-[--radius-xl] bg-gray-50 p-4 md:p-8">
            {reduced ? (
              <img
                ...
                loading="lazy"
                decoding="async"
                className="h-auto w-full max-w-[540px] rounded-[--radius-lg]"
              />
            ) : (
              <video
                className="h-auto w-full max-w-[540px] rounded-[--radius-lg]"
          ...
          <div className="card flex flex-col justify-center px-6 py-8 md:px-8 md:py-10">
        ...
        <div className="mt-16 border-t border-line pt-12 md:mt-24 md:pt-16">
          ...
          <div className="mt-10">
            <button
              ...
              className="-mx-2 inline-flex items-center gap-1.5 rounded-[--radius-sm] px-2 py-1.5 text-[14px] font-semibold text-ink-subtle transition-colors duration-[--dur-fast] ease-[--ease-standard] hover:text-ink hover:underline hover:underline-offset-[5px] motion-reduce:transition-none"
            >
              {openCaveat ? '산정 기준 접기' : '산정 기준 보기'}
              <span
                aria-hidden="true"
                className={`inline-block leading-none transition-transform duration-[--dur-base] ease-[--ease-standard] motion-reduce:transition-none ${
            ...
            <div
              id={caveatId}
              className={`grid transition-[grid-template-rows,opacity] duration-[--dur-base] ease-[--ease-enter] motion-reduce:transition-none ${
                openCaveat ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'
              }`}
            >
              <div className="overflow-hidden" aria-hidden={!openCaveat}>
                <p className="mt-3 max-w-[46rem] rounded-[--radius-lg] bg-canvas-subtle p-5 text-[14px] leading-[1.7] text-ink-subtle md:p-6">
